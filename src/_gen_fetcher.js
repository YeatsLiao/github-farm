...
const CONTRIBUTION_QUERY = \`
  query(\x24username: String!) {
    user(login: \x24username) {
...
export async function fetchContributions(username, token) {
  const response = await fetch(GITHUB_GRAPHQL_URL, {
    method: 'POST',
    headers: {
      'Authorization': \`Bearer \x24{token}\`,
  ...
  if (!response.ok) {
    ...
    throw new Error(\`GitHub API error (\x24{response.status}): \x24{error}\`);
  }

  const { data, errors } = await response.json();
  if (errors) throw new Error(\`GraphQL errors: \x24{JSON.stringify(errors)}\`);
...
`;
fs.writeFileSync('src/fetcher.js', content);
console.log('fetcher.js written');